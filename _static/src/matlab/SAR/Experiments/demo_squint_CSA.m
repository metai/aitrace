...
% ---Step1 azimuth FFT (range dopplor domain)
Sr = fftshift(fft(fftshift(Sr, 1), [], 1), 1);
fas = fa * ones([1, Nr]);

% ---Step2 Phase multiplication1

% ---Step3 range FFT (2D frequency domain)

Sr = fftshift(fft(fftshift(Sr, 2), [], 2), 2);

% ---Step4 Phase multiplication2 ()

% ---Step5 range IFFT (range dopplor domain)

Sr = ifftshift(ifft(ifftshift(Sr, 2), [], 2), 2);

% ---Step6 Phase multiplication2

% ---Step7 azimuth IFFT (2D time domain)
