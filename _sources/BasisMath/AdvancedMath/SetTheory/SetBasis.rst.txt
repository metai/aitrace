...
.. _SubsubSection-SetCharacteristicFunction:

集合的特征函数
~~~~~~~~~~~~~~~~~

.. _defSetCharacteristicFunction:

.. proof:definition:: 集合的特征函数

   设有集合 :math:`\mathbb X` 和 :math:`\mathbb A` , 且 :math:`\mathbb A` 是 :math:`\mathbb X` 的子集, 即 :math:`{\mathbb A} \subset {\mathbb X}` , 称函数
   ...
   .. math::
      ...
      {0\;\;if\;x ∉ \mathbb A}
      \end{array}} \right.
      :label: equ-SetCharacteristicFunction

   为集合 :math:`\mathbb A` 在 :math:`\mathbb X` 中的 **特征函数** (:term:`Characteristic Function`).

.. _SubsubSection-SetCard:

集合的势与基数
~~~~~~~~~~~~~~

在集合论中, 集合 :math:`{\mathbb A}` 的势 (:term:`cardinality`) 用于指集合的元素的个数, 通常记作 :math:`|{\mathbb A}|` 或 :math:`{\rm card} {\mathbb A}` 或 :math:`n({\mathbb A})` .

基数 (:term:`cardinal number`)
